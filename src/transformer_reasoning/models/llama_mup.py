...
class LlamaMuPAttention(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) * self.attn_mult / self.head_dim
...
class LlamaMuPSdpaAttention(LlamaMuPAttention):
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_output = torch.nn.functional.scaled_dot_product_attention(
            ...
            is_causal=is_causal,
            scale=self.attn_mult / self.head_dim,
        )
...
class LlamaMuPDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = LLAMA_MUP_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)
...
class LlamaMuPPreTrainedModel(LlamaPreTrainedModel):
    _no_split_modules = ["LlamaMuPDecoderLayer"]
    ...
    def _init_weights(self, module, readout_zero_init=False, query_zero_init=False):
        ...
        ### muP
        if isinstance(module, LlamaMuPAttention):
            if query_zero_init:
                module.q_proj.weight.data.zero_()

        ### muP
        depth_std = self.config.initializer_range / math.sqrt(2 * self.config.num_hidden_layers)
...
class LlamaMuPModel(LlamaMuPPreTrainedModel):
    """
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaMuPDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class LlamaMuPForCausalLM(LlamaMuPPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    _tp_plan = {"lm_head": "colwise_rep"}

    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaMuPModel(config)
        self.vocab_size = config.vocab_size
